...
class MovieDetailExternalSourceBrokerTest {
    ...
    fun `cuando ambos responden combina resultados correctamente`() = runTest {
        ...
        val tmdbSource = FakeMovieDetailExternalSource(movieToReturn = tmdbMovie)
        val omdbSource = FakeMovieDetailExternalSource(movieToReturn = omdbMovie)
    ...
    fun `cuando solo TMDB responde retorna TMDB con prefijo`() = runTest {
        ...
        val tmdbSource = FakeMovieDetailExternalSource(movieToReturn = tmdbMovie)
        val omdbSource = FakeMovieDetailExternalSource(movieToReturn = null)
        val broker = MovieDetailExternalSourceBroker(tmdbSource, omdbSource)
    ...
    fun `cuando solo OMDB responde retorna OMDB con prefijo`() = runTest {
        ...
        val tmdbSource = FakeMovieDetailExternalSource(movieToReturn = null)
        val omdbSource = FakeMovieDetailExternalSource(movieToReturn = omdbMovie)
        val broker = MovieDetailExternalSourceBroker(tmdbSource, omdbSource)
    ...
    fun `cuando un servicio falla sigue funcionando con el otro`() = runTest {
        val tmdbMovie = buildMovie(title = "Inception", overview = "TMDB Plot")
        val tmdbSource = FakeMovieDetailExternalSource(movieToReturn = tmdbMovie)
        val omdbSource = FakeMovieDetailExternalSource(shouldThrow = true)
        val broker = MovieDetailExternalSourceBroker(tmdbSource, omdbSource)
